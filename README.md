...
The TE profile of a species, that is, the number of copies of each TE in its genome, constitutes a useful feature
to analyze genomic differences between different populations and species, and, along other data, help establish
phylogenetic relationships. REPlotCounts analyzes the TE count matrix (a matrix with the TE profiles of every 
anaylzed species) generated by RECollector to help achieve these goals.
...
The `--exclude` option excludes unknown and other repetitive elements (such as simple repeats
and tRNA genes) from the analysis.
...
### Outputs
- Violin plots for all the TEs that are present in at 75% of the analyzed species. As with REPlotCounts,
there exists an `--exclude` option that excludes unknown and other repetitive elements
(such as simple repeats and tRNA genes) from the analysis.
- Box plots for the selected TE. Groups are color-coded.
